...
export default async function BookmarksPage() {
  ...
  const { data: bookmarks } = await supabase
    .from("bookmarks")
    .select(
      "post_id, posts(id, title, body, category, created_at, cover_image_url, profiles(display_name, username))"
    )
    .eq("user_id", user.id)
    .order("created_at", { ascending: false });

  const posts = (bookmarks ?? []).map((b: any) => b.posts).filter(Boolean);
  ...
  return (
    <div className="max-w-read">
      ...
      <p className="font-body text-body-copy text-grey mb-8">
        Posts you have bookmarked to read later.
      </p>
